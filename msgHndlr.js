...
module.exports = msgHandler = async (hurtz, message) => {
    try {
        ...
        const ownerNumber = '6285155331563@c.us'
    ...
    //try { 
        ...
        // cron.schedule('00 15 * * * *', () => {
        //     const hzbot = '6285155331563-1603688917@g.us'
        //     hurtz.sendText(hzbot, `Terima kasih telah menggunakan jasa Ceria ChatBot grup ini telah expired pada tanggal 52 bot akan otomatis leave terima kasih.`)
...
`
		...
		case 'iri':
		case 'Iri':
              hurtz.sendPtt(from, './media/iri.mp3' ,id)
            break
		case 'tarekses':
		case 'tariksis':
		case 'tarek ses':
		case 'tarik sis':
			  hurtz.sendPtt(from, './media/tarekses.mp3', id)
			break
        ...
        case '!graffity':
            ...
            if (!graffity) return hurtz.reply(from, 'Kirim perintah #graffity teks\n\nContoh #graffity Miku', id)
            const graffitystrz = await get.get('http://inyourdream.herokuapp.com/graffity?kata=' + graffity).json()
            hurtz.reply(from, mess.wait, id)
            hurtz.sendFileFromUrl(from, graffitystrz.status, id)
            break
        case '#Phub':
            hurtz.reply(from, mess.wait, id)
            const phubs = body.slice(6)
            const phubs1 = phubs.split('|')[0]
            const phubs2 = phubs.split('|')[1]
            hurtz.sendFileFromUrl(from, `https://api.vhtear.com/pornlogo?text1=${phubs1}&text2=${phubs2}&apikey=dims2k20`, 'phubs.jpg', 'Nih.....', id)
            break
...
if (isMedia) {
        ...
        */
        ...
        case switch_pref+'read':
            if (!isGroupMsg) return hurtz.reply(from, menuPriv, id)
            if (args.length === 1 || args.length > 2) return hurtz.reply(from, 'Kirim perintah *!read* _Id postingan Ceria_', id)
                try {
                    hurtz.reply(from, mess.wait, id)
        ...
        case switch_pref+'playstore':
            //https://api.vhtear.com/playstore?query=ff&apikey=dims2k20
            if (!isGroupMsg) return hurtz.reply(from, menuPriv, id)
            ...
            try {
                //hurtz.reply(from, '_Sedang mencari data..._', id)
                const dataplay = await get.get(`https://api.vhtear.com/playstore?query=${keywotp}&apikey=dims2k20`).json()
...
◼️ *Akun Private* : ${us[i].is_private ? "✅" : "❌"}
       ...
       //      await hurtz.sendSeen(from)
        ...
        case switch_pref+'donate':
            hurtz.sendLinkWithAutoPreview(from, 'https://saweria.co/ceria', donate)
            await hurtz.sendSeen(from)
            break
        ...
        //         hurtz.reply(from, mess.wait, id)
        //         let urlnulis = "https://api.vhtear.com/write?text="+nulis+"&apikey=dims2k20"              
        ...
        //         const respons = await fetch(`https://api.vhtear.com/ytmp3?query=${serahu}&apikey=dims2k20`)
        ...
        case switch_pref+'getvideo':
            ...
            if (quotedMsg && quotedMsg.type == 'image') {
                ...
                hurtz.reply(from, mess.wait, id)
                //const response1a = await fetch(`https://api.vhtear.com/ytdl?link=https://www.youtube.com/watch?v=${args[1]}&apikey=dims2k20`)
        ...
        case switch_pref+'nyanyi':
            ...
            try {
                hurtz.reply(from, mess.wait, id)
                const datanyanyi = await get.get(`https://api.vhtear.com/music?query=${quernyanyi}&apikey=dims2k20`).json()
...
https://chat.whatsapp.com/HHfql9wXQ7O2b3laFIV1Hm
        ...
        case switch_pref+'tiktok':
            ...
            const jsontik = await get.get(`https://api.vhtear.com/tiktokdl?link=${args[1]}&apikey=dims2k20`).json()
        ...
        case switch_pref+'wiki':
            ...
            //hurtz.reply(from, '_Sedang mencari data..._', id)
            try {
                const wiki = await get.get(`https://api.vhtear.com/wikipedia?query=${encodeURIComponent(query_)}&apikey=dims2k20`).json()
                if (wiki.error) {
        ...
        case switch_pref+'sendowner':
            hurtz.sendContact(from, '6285155331563@c.us')
...
_Hanya isi saja formatnya, dan tidak menghapus apapun!_
        ...
        if (!isGroupMsg) return hurtz.reply(from, menuPriv, id)
         if (isMedia && type === 'image'){
            const ownerNumberFF = '6289696090800@c.us'
...
Nomor : wa.me/${hapusser[0]}
        ...
        //     try {
        //         const teksrin = await get.get(`https://api.vhtear.com/textscreen?query=${keasci}&apikey=dims2k20`).json()
...
➣ *Post* : ${swtich_}
        ...
        //     hurtz.reply(from, `_Sedang mencari data profil..._`, id)
            // const stalk = await get.get(`https://api.vhtear.com/igprofile?query=${args[1]}&apikey=dims2k20`).json()
        ...
        case switch_pref+'artinama':
            ...
            const arte = await get.get('https://api.vhtear.com/artinama?nama='+artihah+'&apikey=dims2k20').json()
        ...
        case switch_pref+'caklontong':
            ...
            const sicak = await get.get('https://api.vhtear.com/funkuis&apikey=dims2k20').json()
        ...
        case switch_pref+'infogempa':
            ...
            const bmkg = await get.get('https://api.vhtear.com/infogempa&apikey=dims2k20').json()
        ...
        const DGCbotowner = 'MRHRTZ@kali:~#'
            */
            ...
            //if (pushname != 'Biancho Junaidi' || pushname != 'MRHRTZ@kali:~#') return hurtz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Ceria Founder dan Owner Bot!', id)
            //if (!isFounder) return hurtz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Ceria Founder dan Owner Bot!', id)
...
💡 Follow Insta Bot Dev : @ceri.962
💌 Contact Me On WhatsApp : @6285155331563
...
╰╼ _CERIA_CHATBOT@3.0 ©2020 ᴍᴀᴅᴇ ʙʏ_ 💗
`
